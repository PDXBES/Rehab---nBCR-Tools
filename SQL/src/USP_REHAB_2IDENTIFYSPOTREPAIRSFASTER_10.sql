...
CREATE PROCEDURE [GIS].[USP_REHAB_2IDENTIFYSPOTREPAIRSFASTER_10] AS
BEGIN
...
BEGIN
	...
	----------------------------------------------------------------------------------------------
	UPDATE  #UnitMultiplierTable SET unit_multiplier = ISNULL(unit_multiplier, 0)+@yearExponent * GIS.NORMDIST(@iterativeYear,Failure_Yr ,Std_Dev,0)/std_dev
	---------------------------------------------------------------------------------------------- 
	SET @iterativeYear = @iterativeYear+1
END
...
FROM	REHAB10FTSEGS 
		INNER JOIN 
		REHAB_Flag5Table 
		ON REHAB10FTSEGS.COMPKEY = REHAB_Flag5Table.COMPKEY
...
/**********************************************/
UPDATE  REHAB_RedundancyTable SET MAT_FmTo = REHAB_RedundancyTable.Material FROM  REHAB_RedundancyTable INNER JOIN REHAB10FTSEGS AS C ON  REHAB_RedundancyTable.MLinkID = C.MLinkID WHERE C.Material <>  REHAB_RedundancyTable.Material
