...
/****** Object:  StoredProcedure [ROSE\issacg].[USP_REHAB_4PREPARETRANSFERTABLEWHOLE_10]    Script Date: 07/21/2011 08:23:33 ******/
SET ANSI_NULLS ON
GO
...
ALTER procedure [ROSE\issacg].[USP_REHAB_4PREPARETRANSFERTABLEWHOLE_10] AS
BEGIN
...
UPDATE  REHAB_RedundancyTableWhole SET  REHAB_RedundancyTableWhole.MLinkID = D.MLinkID FROM (SELECT A.COMPKEY, B.MLinkID FROM  REHAB_RedundancyTableWhole AS A INNER JOIN SANDBOX.GIS.REHAB10FTSEGS AS B ON A.COMPKEY = B.COMPKEY AND B.MLinkID < 40000000) AS D INNER JOIN  REHAB_RedundancyTableWhole ON D.COMPKEY =  REHAB_RedundancyTableWhole.COMPKEY
END
GO
