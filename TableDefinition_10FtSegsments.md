# Purpose of Document #
Detailed Scheme Description of 10FtSegments table

## Input Fields ##
The input fields are defined as those fields which are not to be directly modified or created by the Rehab
processing.  Many of the input fields come from the MstLinks database, which has roots in the MapInfo GIS
system. The data that was retrieved from MstLinks will not always agree with the data found in HANSEN, and
this is known to cause conflicts.  Hopefully in the future, we will be able to remove the MstLinks data from
the process and use only the data from the HANSEN database.

```
  MAINLINES:  Defined by the remarks and mlinkid fields
                  Source: ASM Model
                   Logic: remarks = "BES" & mlinkid < 40000000
```
## Output Fields ##
The output fields are those fields that are modified or inserted by the rehab query process.
...
Field: remarks                         Alias: remarks              Data Type: nvarchar(100)
                                       Nulls: yes                  Unique: no
                                       Index: no                   Default Value: none
                                       Domain: {BES, NULL}         Source: Hansen
       So far, this field can either be ‘BES’ or NULL.  This field indication whether or not BES is the owner
       of and responsible for the pipe in question.
